...
> **Optional:** Set `ISOMER_BOOTSTRAP_PASSWORD` in `.env` on first boot to choose
> the initial `admin` password yourself (the env is read only when the users
> table is empty). Omit it to have the app generate and log a random one.

### 2. Run with docker-compose

The [`docker-compose.yml`](docker-compose.yml) file defines the `isomer` service,
binds the app to `127.0.0.1:27001` (loopback-only), and mounts a named volume
...
On the first boot the container generates a one-time bootstrap password for the
`admin` user and prints it to stderr:
...
The Flask app in [`app.py`](app.py) listens on port **27001** (see the
`app.run(host="0.0.0.0", port=27001)` call at the bottom of the file). After
starting the container, open the dashboard in your browser:

```
http://127.0.0.1:27001/
```

Log in with `admin` and the bootstrap password, then change the password from
**Settings → Users**.

> **Note:** The app binds only to loopback. For remote access, place an nginx
> reverse proxy (see [`deploy/isomer.zoleb.com.conf`](deploy/isomer.zoleb.com.conf))
> in front of it to terminate TLS and apply security headers.
